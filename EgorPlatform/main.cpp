...
void changeLevel(Level& lvl, int& numberLevel) {
	switch (numberLevel)
	{
	case 1: lvl.LoadFromFile("map.tmx"); break;//загрузили в него карту 1, внутри класса с помощью методов он ее обработает.
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (p.isShoot == true) {
				...
				shoot.play();
			}//играем звук пули //если выстрелили, то появляется пуля. enum передаем как int 
		}

		lifeBarPlayer.update(70);//сюда передаем значение, которое надо нарисовать. Можно передать здоровья игрока тогда будет lifeBarPlayer.update(player.getHealth())
		...
		if (Keyboard::isKeyPressed(Keyboard::Escape)) { return false; }//если эскейп, то выходим из игры		if (Keyboard::isKeyPressed(Keyboard::Escape)) { return false; }//если эскейп, то выходим из игры
